...
使用一个father数组，记录父节点，比如father\[1] = 1,1的父节点就是自己，即1是根节点，利用不同的树区分不同的集合
...
+ 对于第一步中获取的两个节点，只需要使其中一个指向另一个，即father【A】 = B
```c
...
上面讲述的并查集操作是最简单的思想，举个例子，如果给出的集合是一条链
那么寻找父节点的过程计算量就非常大，所以要进行优化
看下面四个节点
...
如果只是想寻找父节点，直接把四个值全赋值成1即可

此时树型从一条链变成了一把伞
装换的过程可以概括为下面两个步骤
+ 按之前的方法获得x的根节点
+ 重新从x走一遍寻找根节点的过程，把路径上的每一个节点父节点赋值根节点
```c
int findFather(int x) {
    int a = x;
    while (x != father[x]) {
...
> 已知如果A和B是好朋友，等价于B和A是好朋友，如果A和C是好朋友，B和C也是好朋友，那么A和B也是好朋友。现给出一些人，问能够分成几组，使得每组之内两两都是好朋友，每组之间不存在好朋友,输入两个正整数n，m，n为人数，m为朋友对数，然后分别输入m行，每行是两个互为朋友的编号  ，输出组数
...
堆是一棵完全二叉树，父子节点有序，每个父节点的值不大于或者不小于自己诶单的值
+ 如果每个节点的值都是以根节点为子树的最大值，那么称为大顶堆，即根节点为整棵树的最大值
+ 相反，如果每个节点的值都是以根节点为子树的最小值，称之为小顶堆，即根节点是整棵树的最小值

对于一个无序的完全二叉树，通常使用向下调整来有序化成一个堆，以构建大顶堆为例，思路为：
总是将当前节点V与其左右孩子比较（如果有的话），如果孩子中存在大于结点V的，就与孩子结点交换位置，
交换完之后，继续与孩子节点比较，重复这一过程

结合之前的完全二叉树的储存方法，代码如下

```c
const int maxn = 100;
int heap[maxn], n = 10;

void downJust(int low,int high) {   //堆的数组范围[low,high]
    int i = low, j = i * 2;
    while (j <= high) {
...
删除堆顶元素，需要先把堆尾的元素替换堆顶的元素，然后再一次向下调整到有序
...
把节点的权值乘以路径长度称为带权路径长度，所有节点的带权路径长度称为树的带权路径长度（WPL）

使得带权路径长度最小的树称为哈夫曼树;
对同一组节点来说，哈夫曼树可能不唯一，但是树的最小带权路径长度一定唯一

下面给出一个简洁的方法构造哈夫曼树：
+ 初始状态下只有n个节点，将他们视为n个只有一个结点的树
+ 合并其中根节点权值最小的两棵树，生成的父结点的权值为二者的和，这样树的数量就减少了一棵
+ 重复第二步，直至剩下惟一一棵树，得到哈夫曼树
...
**前缀编码**： 任何一个字符的编码都不会是其他字符的前缀

将哈夫曼树中的所有左右子树分别标为0和1,，对于每个叶子节点的编号即为前缀编码

哈夫曼树组成的前缀编码时能使01串最短的前缀编码，这种由哈夫曼树导出的前缀编码也称为哈夫曼编码

# 2019年02月22日打卡

# 图

图由节点和边组成，每条边的两端必须是图的两个节点，一般分为有向图和无向图
+ 有向图： 所有边都有方向
+ 无向图： 所有边默认为双向

+ 顶点的度： 该顶点连接的边数；于有向图来说，出边数称为出度，入边数称为入度
+ 边和节点量化的属性为权值
## 图的储存

### 邻接矩阵
设图的顶点标号为0,1，，，N-1，令二维矩阵G【N】【N】表示每两个节点之间的组合，如果对应值为1（或边权值），则说明对应两个节点之间存在边
用特定值表示不存在边（不可达），适用于顶点数目不超过1000的图

### 邻接表
把一个结点的所有出边放在一个列表中，则N个节点的图就会有N个列表，记为adj【N】，列表以链表的方式实现，即每个adj【i】储存的是出边链表头结点的地址，表示节点i，而其余边依次相接

**adj\[0]->1(2)->4(1)**

这就表示节点0有一条指向结点1的边，权值是2,；此外结点0还有一条指向结点4的边，权值为1；
可以看出adj对应的位置中是一个头结点，依次相接的结点是由两个值域和一个指针域组成，最末的一个结点指针域为空

+ 除了使用链表实现邻接表之外，也可以用更简单的vector
